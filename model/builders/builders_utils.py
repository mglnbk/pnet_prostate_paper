...
class pnet(Model):
  def __init__(self, inputs, features, genes, n_hidden_layers, direction, activation, activation_decision, w_reg,
    ...
    w_reg = list(w_reg)
    w_reg_outcomes = list(w_reg_outcomes)
    ...
    if sparse:
        if shuffle_genes == 'all':
            ...
            self.layer1 = SparseTF(n_genes, mapp, activation=activation, kernel_regularizer=reg_l(w_reg0),
                              name='h{}'.format(0), kernel_initializer=kernel_initializer, use_bias=use_bias,
                              **constraints)
        else:
            self.layer1 = Diagonal(n_genes, input_shape=(n_features,), activation=activation, kernel_regularizer=l2(w_reg0),
        ...
        if sparse_first_layer:
            ...
            self.layer1 = Dense(n_genes, input_shape=(n_features,), activation=activation, kernel_regularizer=l2(w_reg0),
                           use_bias=use_bias, name='h0', kernel_initializer=kernel_initializer)
    outcome = self.layer1(inputs)
    if attention:
        self.attention_probs = Diagonal(n_genes, input_shape=(n_features,), activation='sigmoid', kernel_regularizer=l2(w_reg0),
                                   name='attention0')(inputs)
        outcome = multiply([outcome, self.attention_probs], name='attention_mul')
    ...
    self.decision_outcome = Dense(1, activation='linear', name='o_linear{}'.format(0), 
    ...
    # testing
    if batch_normal:
        self.decision_outcome = BatchNormalization()(self.decision_outcome)
    # if reg_outcomes:
    ...
    self.decision_outcome = Dense(1, activation='linear', name='o_linear{}'.format(1),
                             kernel_regularizer=reg_l(w_reg_outcome1 / 2.))(outcome)

    drop2 = self.Dropout(dropout[0], name='dropout_{}'.format(0))
    ...
    decision_outcome = Activation(activation=activation_decision, name='o{}'.format(1))(self.decision_outcome)
    ...
    if n_hidden_layers > 0:
        ...
        for i, mapp in enumerate(maps[0:-1]):
            ...
            if attention:
                ...
                self.hidden_layers.append(attention_probs)
            ...
            decision_outcome = Dense(1, activation='linear', name='o_linear{}'.format(i + 2),
                                     kernel_regularizer=reg_l(w_reg_outcome))
            
            if batch_normal:
                decision_outcome = BatchNormalization()(decision_outcome)
            self.hidden_layers.append(decision_outcome)
...
def get_pnet(inputs, features, genes, n_hidden_layers, direction, activation, activation_decision, w_reg,
    ...
    # if reg_outcomes:
    # decision_outcome = Dense(1, activation='linear', name='o_linear{}'.format(0), kernel_regularizer=reg_l(w_reg_outcome0), **constraints)(inputs)
    decision_outcome = Dense(1, activation='linear', name='o_linear{}'.format(0), 
                             kernel_regularizer=reg_l(w_reg_outcome0))(inputs)
    # else:
    #     decision_outcome = Dense(1, activation='linear', name='o_linear{}'.format(0))(inputs)
